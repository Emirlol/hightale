...
define_packet! {
	BuilderToolArg {
		fixed {
			...
			opt(0, 8) brush_shape_arg: BuilderToolBrushShapeArg,
			opt(0, 16) brush_origin_arg: BuilderToolBrushOriginArg,
			opt(0, 32) brush_axis_arg: BuilderToolBrushAxisArg,
			opt(0, 64) rotation_arg: BuilderToolRotationArg,
		}
		variable {
			opt(0, 128) string_arg: BuilderToolStringArg,
			opt(1, 1) block_arg: BuilderToolBlockArg,
			opt(1, 2) mask_arg: BuilderToolMaskArg,
...
define_packet! {
	BuilderToolBrushData {
		fixed {
			...
			opt(1, 4) use_mask_commands: BuilderToolBoolArg,
			opt(1, 8) invert_mask: BuilderToolBoolArg,
		}
		variable {
			opt(1, 16) material: BuilderToolBlockArg,
			opt(1, 32) favorite_materials: Vec<BuilderToolBlockArg>,
			opt(1, 64) mask_arg: BuilderToolMaskArg, // Funny enough, making this "mask" causes a name conflict with the generated byte mask
			opt(1, 128) mask_above: BuilderToolMaskArg,
			opt(2, 1) mask_not: BuilderToolMaskArg,
			opt(2, 2) mask_below: BuilderToolMaskArg,
			opt(2, 4) mask_adjacent: BuilderToolMaskArg,
			opt(2, 8) mask_neighbor: BuilderToolMaskArg,
			opt(2, 16) mask_commands: Vec<BuilderToolStringArg>,
		}
	}
}
...
define_packet! {
	BuilderToolSelectionTransform {
		fixed {
			opt(1) initial_selection_min: Vector3i,
			opt(2) initial_selection_max: Vector3i,
			opt(4) initial_rotation_origin: Vector3f,
			required cut_original: bool,
			required apply_transformation_to_selection_min_max: bool,
			required is_exiting_transform_mode: bool,
			opt(8) initial_paste_point_for_clipboard_paste: Vector3i,
		}
		variable {
			opt(16) transformation_matrix: Vec<f32>
		}
	}
}
