...
impl Parse for MaskedField {
	fn parse(input: ParseStream) -> syn::Result<Self> {
		...
		let kind = match ident_str.as_str() {
			"required" | "req" => MaskedFieldKind::Required,
			"optional" | "opt" => {
				let content;
			...
			_ => {
				return Err(syn::Error::new(ident.span(), "Expected 'required' or 'opt' keyword for MaskedField"));
			}
		};
		let name: Ident = input.parse()?;
		let _colon_token: Token![:] = input.parse()?;
		...
		let explicit_padding = {
			...
			if bracket_start.peek(syn::token::Bracket) {
				...
				let pad_ident: Ident = content.parse()?;
				let pad_ident_str = pad_ident.to_string();
				if pad_ident_str != "pad" {
					return Err(syn::Error::new(pad_ident.span(), "Expected 'pad' keyword in padding specification"));
				}
...
impl Parse for PacketBody {
	fn parse(input: ParseStream) -> syn::Result<Self> {
		let is_masked = if input.peek(syn::Ident) {
			...
			s == "mask_size" || s == "fixed" || s == "variable"
		} else {
			false
		};
		if !is_masked {
		...
		while input.peek(syn::Ident) {
			...
			match ident_str.as_str() {
				...
				_ => unreachable!(), // Already handled above for Sequential fields
			}
		}
...
pub fn generate(packet_def: PacketDefinition) -> proc_macro2::TokenStream {
